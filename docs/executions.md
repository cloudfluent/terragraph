...
Ordinary `plan`, `apply`, and `destroy` JSON results expose the existing session ID as `execution_id`, including when a later node or journal update fails. `apply --plan` returns the selected execution's ID. Saved-plan and history results keep `executions[].id`; `plan recover` keeps its flat `id`. These are the same IDs accepted by `plan show`, not separate invocation IDs. Commands such as `validate`, `graph`, `status`, `output`, and `vendor` do not create execution records or IDs.

A failure before session acquisition omits `execution_id`. A barrier caused by an earlier uncertain execution identifies that earlier record in `diagnostics[].related_execution_id`. If initial journal creation fails after an ID is allocated, that diagnostic can also carry the allocated ID; its presence does not prove the record was persisted. Inspect the store before retrying.

Recover and cancel failures retain any known record with its last confirmed persisted status. A failed publication never reports the intended next status as successful. `plan prune` keeps any already removed IDs and adds diagnostics when subsequent cleanup fails. Global journal failures remain visible alongside node failures; they must not be treated as evidence that no mutation occurred.
...
To freeze producers instead, start with `terragraph plan --save --node checkout.cluster --upstream --output json`; use the same saved apply and continuation commands without selection flags.
