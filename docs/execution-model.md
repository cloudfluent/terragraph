...
`sensitive`. Both of apply's branches write it: a node that just changed and a
node that plans clean describe the same current reality.
...
A snapshot read also checks the current module schema, so a legacy file cannot
