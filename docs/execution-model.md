...
owner-only, deterministic bytes) containing **only the outputs a data edge
actually consumes from that node** — a value no edge reads is publishable
surface with zero consumers. Both of apply's branches write it: a node that
just changed and a node that plans clean describe the same current reality.
...
changes what gets torn down. A missing or corrupt snapshot is never an
error; a fallback that errors is a second source of truth.
