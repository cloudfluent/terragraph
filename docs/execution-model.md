...
A deadline does not establish whether infrastructure changed. The existing journal records observed outcomes and retains indeterminate mutations for explicit recovery; timeout never retries a mutation. Successfully completed actions retain their success even if cancellation arrives at the completion boundary. Subprocess cleanup can extend elapsed time beyond the deadline. Unix waits for its process group. Windows assigns timed runtimes to a non-breakaway job before resuming the whole process and normally waits for tracked processes to terminate. Windows shutdown has a separate five-second grace period: if termination fails or accounting does not settle, it attempts to kill tracked processes, closes the kill-on-close job, detaches output buffers, and returns a cleanup/deadline error instead of retaining the action slot and graph lock indefinitely. Such an error does not prove that mutation stopped; inspect the existing execution journal and follow explicit recovery before retrying.
