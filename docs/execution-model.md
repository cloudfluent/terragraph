...
- **Error**: blocks the command (a `from`/`to` referencing a port that doesn't exist, two data edges targeting the same input after group expansion, a data edge and `vars` both setting the same input, a cycle, a value that doesn't fit the target variable's declared type).
