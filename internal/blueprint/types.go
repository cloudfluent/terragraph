...
const (
	...
	// TFVarsLocationModule writes to <node source>/.terragraph.<node>.tfvars.json, alongside the module's own .tf files, so a node's resolved input values sit next to the source while it runs. The file is removed when the run ends, in this location as in the other — it holds resolved inputs in cleartext — so this chooses only where the file lives during the run, and it is the weaker choice: the same secrets land inside a source tree someone may commit. Requires the module's .gitignore to exclude the engine-managed pattern (see docs/execution-model.md); terragraph validate warns about a stale file left behind by a since-renamed or since-removed node sharing that directory, but never deletes one on your behalf.
