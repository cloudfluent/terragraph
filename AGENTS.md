...
- **terragraph writes exactly three kinds of file:** the gitignored ephemeral tfvars (`internal/exec`), the vendor manifest (`internal/vendor`), and the gitignored opt-in output snapshots at `.terragraph/outputs/<node>.json` (`internal/engine`, see docs/execution-model.md — local, per-machine, regenerated by apply, and consumed only as the input source of last resort). It never generates or edits a `.tf` file, and it never writes into a module's own directory. A new write path outside those three is a design change, not an implementation detail.
